...
export default function ScoreCardDetailsCompleted({ matchData }){

    const latestInningsData = matchData?.scorecardSummary?.innings?.slice(-1);

    return(<>
        <div className="surface w-full text-base">
            <div className="flex justify-center py-2">
                <span className="text-slate-500">{matchData?.match?.series?.name}, {matchData?.match?.season} · {matchData?.match?.title} · {matchData?.match?.format}</span>
            </div>
                ...
                <div className="flex-auto text-center">
                    ...
                    <div className="inline-flex rounded-md" role="group">
                        <div className="font-bold">
                            {formatDate(matchData.match.startTime)} • {matchData?.match?.status}
                        </div>
                    </div>
